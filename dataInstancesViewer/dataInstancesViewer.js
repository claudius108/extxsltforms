...
function dataViewer_getModelsIDs() {
	...
	, dataInstancesViewerDiv = document.getElementById('dataInstancesViewerDiv');
	...
	if (showViewerButtonValue.nodeValue == 'Expand Data Instances Viewer') {
		...
		var models = XsltForms_globals.models
		, modelsOptionString = ''
		, instancesDivString = ''
		;
		...
		for (var i=0; i < models.length; ++i) {
			var model = models[i]
			, modelId = model.element.id;
			if (modelId != 'xsltforms-mainform-model-config' && modelId != 'xf-model-extensions') {
...
function dataViewer_getInstance(object) {
	...
	, selectedModelId = modelsSelect.options[modelsSelect.selectedIndex].value
	, instanceID = object.options[object.selectedIndex].value
	, xmlXNode = document.getElementById(instanceID).xfElement.doc
	, xmlString = XsltForms_browser.saveXML(xmlXNode)
	, xsltResult = eXSLTForms_xslt(xmlString, exsltforms.utils.baseURI + 'dataInstancesViewer/mozDss5.xsl', null)
	;
